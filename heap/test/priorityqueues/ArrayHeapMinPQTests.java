...
public class ArrayHeapMinPQTests extends BaseTest {
    ...
    @Nested
    @DisplayName("Add 10 Decreasing Priority")
    class Add10Decreasing {
        ...
        void removeMinRepeatedly_returnsAllItems() {
            ...
            assertThat(output).containsExactlyInAnyOrder(correctOrdering);
            assertThat(pq).isValid();
        }
    }
    ...
    class Add10Arbitrary extends Add10Increasing {
        ...
        @Test
        void arbitrary_removesRepeatedly() {
            ExtrinsicMinPQ<String> pq = setUpMinPQ();
            assertThat(pq).as("invariant check before removing all elements").isValid();
            List<String> output = removeAll(pq);
            assertThat(output).containsExactly(correctOrdering);
    ...
    @Nested
    @DisplayName("our test for decreaing priority")
    class Decreasing extends Add10Increasing {
        Decreasing() {
            this.correctOrdering = new String[]{"Grantaire", "Enjolras", "Thenardier", "Fantine", "Gavroche",
                "Eponine", "Marius", "Cosette", "Javert", "Jean Valjean"};
        }

        @Override
        ExtrinsicMinPQ<String> setUpMinPQ() {
            ExtrinsicMinPQ<String> pq = createMinPQ();
            pq.add("Jean Valjean", 10);
            pq.add("Javert", 9);
            pq.add("Cosette", 8);
            pq.add("Marius", 7);
            pq.add("Eponine", 6);
